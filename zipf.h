...
public:
    ZipfRequests(std::string sizeFile, int64_t objCount, double zipfAlpha) {
        ...
        std::cerr << "debug " << popDist.size() << " " << popDist[popDist.size()-1].size() << " effective probability " << 1.0/popDist.size() * 1.0/popDist[popDist.size()-1].size() << " theoretical probability " << 1/pow(double(objCount), zipfAlpha) << "\n";
    }
    ...
    void Sample(SimpleRequest * req) {
        ...
        const int64_t id=sample.first;
        const int64_t size=sample.second;
        req->reinit(id,size); // id, size
        if(stats.count(id)>0) {
            stats[id].second++;
        } else {
            stats[id].first = size;
